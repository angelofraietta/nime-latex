...
\newtoggle{anonbibliography}
%you have to watch out here because if you are doing the swap on your bibliography, it appears cached, so you need to do clean all auxilary files and then compile again
\toggletrue{anonbibliography}
%\togglefalse{anonbibliography}

\iftoggle{anonbibliography}{%
...
\anonymize{ACM SIG Proceedings}. It is an {\em alternate} style which produces
...
You will note that \cite{mypublication} and \cite{myotherpublication} were written by me, so in blind mode, it will use the \texttt{personal-anon.bib} file. 
