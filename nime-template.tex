...
Additionally, you will need to make a copy of your personal references, using the second version with a key name adding \texttt{-hidden}. For example, if your reference key is \texttt{myotherpublication}, make a copy called \texttt{myotherpublication-hidden} that has text displayed how you would like it displayed in blind review mode. These have been named in this example \texttt{personal.bib} and \texttt{personal-anon.bib}. Mask all identifying information in the \texttt{personal.bib} file, however, keep the citation key the same as that in your \texttt{personal.bib}.
You will note that \anonybib{mypublication} and \anonybib{myotherpublication} were written by me, so in blind mode, it will use the \texttt{personal-anon.bib} file. When switching to and from \texttt{blind} mode, you may need to delete your auxiliary files or do a bibliography refresh to clear the \LaTeX\ cache..
